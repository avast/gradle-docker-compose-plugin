> The latest released version is `0.17.7`. Sonatype support confirmed that they accidentaly deleted the token used for publising, so we are waiting for a new token.
