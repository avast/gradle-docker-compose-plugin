...
2. It's recommended not to assign fixed forwarded ports in `docker-compose.yml` for automated builds because it can cause ports collision on integration server. So exported ports of your application are mapped to random ports. This plugin reads these ports (and even IP addresses of containers) and exposes them in `dockerCompose.servicesInfo` map.
